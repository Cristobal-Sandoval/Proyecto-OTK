...
import { validateCommunityApplication, readJson, send } from './_lib/validate.js';
import { listCommunityApps, addCommunityApp, removeCommunityApp } from './_lib/store.js';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    return send(res, 200, { applications: await listCommunityApps() });
  }

  // POST = postulación pública con validación + rate-limit
  if (req.method === 'POST') {
    const rl = rateLimit(`community-apply:${clientIp(req)}`, 5, 60 * 60 * 1000);
    if (!rl.allowed) {
    ...
    const v = validateCommunityApplication(body);
    ...
    const created = await addCommunityApp(app);
  ...
  if (req.method === 'DELETE') {
    ...
    const removed = await removeCommunityApp(id);
    if (!removed) return send(res, 404, { error: 'No encontrada.' });
    return send(res, 200, { ok: true, applications: await listCommunityApps() });
  }
